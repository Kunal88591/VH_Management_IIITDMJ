...
import { HiCheckCircle, HiClock, HiCalendar, HiUser, HiHome, HiUserGroup } from 'react-icons/hi';
          ...
          <div className="bg-gray-50 rounded-lg p-6 text-left mb-6">
            ...
            <div className="space-y-3">
              <div className="flex items-start">
                <HiUserGroup className="w-5 h-5 text-secondary mr-3 mt-0.5" />
                <div className="flex-1">
                  <p className="text-sm text-gray-500">Guests ({booking.numberOfGuests || booking.guests?.length || 1})</p>
                  <p className="font-medium">{guestNames}</p>
                </div>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">₹{booking.roomCharges || 0}</span>
              </div>
              {booking.mealRequirements && booking.mealRequirements.required && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Meal Charges</span>
                  <span className="font-medium">₹{booking.mealCharges || 0}</span>
                </div>
              )}
              <hr className="my-2" />
              <div className="flex items-center justify-between">
                <span className="text-gray-700 font-medium">Total Amount</span>
                <span className="text-xl font-bold text-secondary">₹{booking.totalAmount || 0}</span>
              </div>
            </div>
          </div>
